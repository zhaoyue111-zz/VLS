"""Full-window vision-language uncertainty-gated pseudo-label fusion.

A frozen six-action vision World Predictor and a frozen pre-projection
Language WP produce ten probability predictions. Their population variance
gates a soft interpolation from the original hard source pseudo-label to the
ten-member mean. GT is diagnostic/evaluation only.
"""
...
OUTPUT_DIR = Path("output_2/v2_vision_language_uncertainty_weighted_pseudo")
DEFAULT_WP_ACTIONS = (
    "gamma:-0.30",
    "gamma:+0.30",
    "gaussian_noise:+0.02",
...
LANGUAGE_PROMPTS = ("the liver", "human liver", "hepatic organ")
PREDICTION_MEMBER_NAMES = (
    "source",
    "vision_gamma_-0.30",
    "vision_gamma_+0.30",
    "vision_gaussian_noise_+0.02",
    "vision_gaussian_noise_+0.05",
    "vision_bias_field_+0.05",
    "vision_bias_field_+0.15",
    "language_the_liver",
    "language_human_liver",
    "language_hepatic_organ",
)
PREDICTION_COUNT = len(PREDICTION_MEMBER_NAMES)
...
def load_runtime_dependencies() -> None:
    ...
    global flatten_prompt_embedding
    ...
    from vls.v10_language_wp_shared_output import LanguageResidualWP, flatten_prompt_embedding
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Full-window vision-language uncertainty-gated pseudo-label fusion."
    )
    ...
    parser.add_argument(
        "--calibration-only",
        action="store_true",
        help="Calibrate the unified ten-member vision-language ensemble on train tiles, then stop before SFDA.",
    )
...
def load_language_world_predictor(
    checkpoint_path: Path,
    device: torch.device,
) -> tuple[torch.nn.Module, dict[str, Any]]:
    if not checkpoint_path.exists():
        raise FileNotFoundError(f"Language WP checkpoint not found: {checkpoint_path}")
    checkpoint = torch.load(checkpoint_path, map_location="cpu", weights_only=False)
    if checkpoint.get("stage") != "V10 pre-projection shared-output Language WP":
        raise AssertionError(f"Unexpected Language WP checkpoint stage: {checkpoint.get('stage')}")
    if checkpoint.get("source_prompt") != SOURCE_PROMPT:
        raise AssertionError(f"Language WP source prompt is not {SOURCE_PROMPT!r}")
    trained_targets = tuple(str(value) for value in checkpoint.get("target_prompts", ()))
    if trained_targets != LANGUAGE_PROMPTS:
        raise AssertionError(
            f"Language WP targets differ: trained={trained_targets}, expected={LANGUAGE_PROMPTS}"
        )
    model = LanguageResidualWP(
        shared_dim=int(checkpoint["shared_dim"]),
        text_delta_dim=int(checkpoint["text_delta_dim"]),
    ...
    metadata = {
        "path": str(checkpoint_path),
        "stage": checkpoint["stage"],
        "source_prompt": checkpoint.get("source_prompt"),
        "target_prompts": list(trained_targets),
        "shared_dim": int(checkpoint["shared_dim"]),
        "text_delta_dim": int(checkpoint["text_delta_dim"]),
        "hidden_dim": int(checkpoint["hidden_dim"]),
        "selected_epoch": checkpoint.get("selected_epoch"),
...
def imagined_predictions(
    ...
    torch.Tensor,
    list[tuple[WpAction, torch.Tensor]],
    ...
    language_outputs: list[tuple[str, torch.Tensor]] = []
    shared_source = source_context["shared_prompt_outputs"][:, 0].detach().float()
    decoder = source_context["decoder_audit"]["decoder"]
    source_shape = tuple(source_probability.shape)
    for prompt, delta in zip(LANGUAGE_PROMPTS, language_text_deltas, strict=True):
        predicted_shared = language_world_predictor(
            shared_source,
            delta[None].to(device=device, dtype=torch.float32),
        )
        autocast = torch.autocast(device.type, enabled=True) if device.type == "cuda" else nullcontext()
        with autocast:
            mask_embeddings = [
                projection(predicted_shared[:, None].float())
                for projection in network.project_to_decoder_channels
            ]
            predicted_logits = decoder(
                source_context["decoder_audit"]["skips"], mask_embeddings,
            )
            if isinstance(predicted_logits, (list, tuple)):
                predicted_logits = predicted_logits[0]
        if tuple(predicted_logits[:, :1].shape) != source_shape:
            raise AssertionError(
                f"Language decoder output shape {tuple(predicted_logits[:, :1].shape)} "
                f"does not match source shape {source_shape}"
            )
        language_outputs.append((prompt, torch.sigmoid(predicted_logits[:, :1].detach().float())))
        del predicted_shared, predicted_logits, mask_embeddings
    if len([source_probability, *[prediction for _, prediction in outputs], *[prediction for _, prediction in language_outputs]]) != PREDICTION_COUNT:
        raise AssertionError("Vision-language ensemble must contain exactly 10 predictions")
    return source_probability, outputs, language_outputs


def combined_predictions(
    source_probability: torch.Tensor,
    vision_outputs: list[tuple[WpAction, torch.Tensor]],
    language_outputs: list[tuple[str, torch.Tensor]],
) -> list[torch.Tensor]:
    predictions = [
        source_probability,
    ...
    if len(predictions) != PREDICTION_COUNT:
        raise AssertionError(
            f"Expected {PREDICTION_COUNT} predictions {PREDICTION_MEMBER_NAMES}, got {len(predictions)}"
        )
    reference_shape = tuple(predictions[0].shape)
    if any(tuple(prediction.shape) != reference_shape for prediction in predictions):
        raise AssertionError("All vision-language prediction shapes must match")
    if any(prediction.dtype != torch.float32 for prediction in predictions):
        raise AssertionError("All vision-language predictions must be float32")
    return predictions
    ...
    for case_index, case in enumerate(cases):
        ...
        for patch_index, slicer in enumerate(slicers):
            ...
            source_probability, vision_outputs, language_outputs = imagined_predictions(
                interface, network, world_predictor, language_world_predictor, context,
            ...
            raw, ensemble = prediction_moments(predictions)
            sampled_by_group["combined"].append(calibration_sample(
            ...
            del patch, context, source_probability, vision_outputs, language_outputs, predictions, raw, ensemble
    ...
    for epoch in range(1, args.training_epochs + 1):
        for case_position, case_index in enumerate(rng.permutation(len(train_cases)).tolist(), start=1):
            ...
            for slicer, coverage_weight in zip(slicers, coverage_weights, strict=True):
                ...
                del vision_outputs, language_outputs, predictions
...
def run(args: argparse.Namespace) -> None:
    ...
    if tuple(action.label for action in actions) != DEFAULT_WP_ACTIONS:
        raise AssertionError(
            "V2 requires exactly the six V1 vision actions in their default order; "
            f"got {[action.label for action in actions]}"
        )
    ...
    if args.device == "cuda" and device.type != "cuda":
        raise RuntimeError(f"V2 vision-language uncertainty pseudo-label fusion requires CUDA, resolved {device}")
    ...
    language_embedding = interface.embed_text_prompts(
        [SOURCE_PROMPT, *LANGUAGE_PROMPTS],
    ).detach().cpu()
    source_text = flatten_prompt_embedding(language_embedding, 0)
    language_text_deltas = torch.stack([
        flatten_prompt_embedding(language_embedding, index) - source_text
        for index in range(1, len(LANGUAGE_PROMPTS) + 1)
    ]).to(device)
    ...
    language_world_predictor, language_checkpoint_metadata = load_language_world_predictor(
    ...
    calibration_scale, calibration_stats, manifest = calibrate_uncertainty(
        ...
        train_cases, embedding, text_delta, language_text_deltas, actions, args, device,
    )
    ...
    if args.calibration_only:
        diagnostic_summary = {
            "stage": "V2 vision-language WP disagreement calibration",
            "status": "calibration_complete",
            "sfda_run": False,
            "gt_used": False,
            "prompt_actions_used": True,
            "blur_used": False,
            "language_actions_used": list(LANGUAGE_PROMPTS),
    ...
    for variant in VARIANTS:
        losses, pseudo_rows, full_rows, stats = train_variant(
            variant, base_network, world_predictor, language_world_predictor,
            train_cases, eval_cases, full_data, interface, embedding, text_delta,
            language_text_deltas, actions,
    ...
    summary = {
        "stage": "V2 full-window vision-language uncertainty-gated pseudo-label fusion",
        ...
        "prediction_member_names": list(PREDICTION_MEMBER_NAMES),
        "prediction_contract": {
            "members": list(PREDICTION_MEMBER_NAMES),
            "shape_check": "all 10 probability tensors have identical shape",
            "dtype_check": "all 10 probability tensors are float32",
            "decoder_check": "language predictions use frozen project_to_decoder_channels and native decoder",
            "backward_check": "LoRA receives finite gradients during training",
        },
        "wp_actions": [action.label for action in actions],
        ...
        "formula": {
            "prediction_stack": (
                "[source:liver + 6 vision WP predictions + 3 language WP "
                "predictions for the configured correct language actions]"
            ),
        ...
        "training": {
            ...
            "vision_world_predictor_frozen": True,
            "language_world_predictor_frozen": True,
