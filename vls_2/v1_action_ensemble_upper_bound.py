"""Action-ensemble hard pseudo-label upper bound for the V1 pipeline.
...
* C_wp_hard_fusion: mean(source + six imagined WP predictions), thresholded;
* D_real_action_hard_fusion: mean(source + six real-action predictions),
  thresholded.

Only the frozen source model creates the pseudo labels.  GT is read after that
step for diagnostics and final evaluation only.  Real actions are generated
from the complete raw case, one action at a time, and V0's stable
case/action/seed random-field implementation is reused.
"""
...
from vls_2.v0_mri_action_screening import (
    ActionVariant,
    apply_action,
    assert_action_geometry_unchanged,
    preprocess_image,
)


OUTPUT_DIR = Path("output_2/v1_action_ensemble_upper_bound")
VARIANTS = (
...
REAL_ACTION_LABELS = WP_ACTION_LABELS
...
class RealAction:
    ...
    def v0_variant(self) -> ActionVariant:
        ...
        raise ValueError(f"Unsupported real action family: {self.family}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--real-actions",
        default=",".join(REAL_ACTION_LABELS),
        help="Must remain the six V0-fixed real MRI actions for a comparable formal run.",
    )
    parser.add_argument(
        "--smoke-only",
        action="store_true",
...
def parse_real_actions(value: str) -> tuple[RealAction, ...]:
    labels = parse_labels(value)
    actions: list[RealAction] = []
    for label in labels:
        family, separator, strength = label.partition(":")
        if separator != ":":
            raise ValueError(f"Invalid action {label!r}; expected family:strength")
        actions.append(RealAction(family, float(strength)))
    expected = set(REAL_ACTION_LABELS)
    if set(action.label for action in actions) != expected:
        raise ValueError(f"Real actions must be exactly {sorted(expected)}")
...
def load_case_cache(cache: dict[str, Any]) -> dict[str, Any]:
    ...
    if payload.get("stage") != "V1 action ensemble uint8 pseudo cache":
...
def train_variant(
    ...
    for epoch in range(1, args.training_epochs + 1):
        ...
        for case_position, case_index in enumerate(epoch_indices, start=1):
            ...
            print(
                f"[v1_action_upper_bound] {variant} epoch={epoch}/{args.training_epochs} "
...
def run(args: argparse.Namespace) -> None:
    ...
    real_actions = parse_real_actions(args.real_actions)
    assert_equal_weight_fusion_contract(len(wp_actions))
    if len(wp_actions) != len(real_actions):
        raise AssertionError("WP and real action counts must match")
    if args.self_test_only:
        print(json.dumps({
            "status": "self_test_passed",
            "prediction_count": 1 + len(wp_actions),
            "weight_per_prediction": 1.0 / float(1 + len(wp_actions)),
            "wp_actions": [action.label for action in wp_actions],
            "real_actions": [action.label for action in real_actions],
        }, indent=2))
        return
    ...
    for case in train_cases:
        cache, actions, geometry, pseudo = build_case_target_cache(
            case, interface, base_network, world_predictor, embedding, text_delta,
            wp_actions, real_actions, cache_dir, args, device,
        )
        ...
        print(
            f"[v1_action_upper_bound] cached {case.case} "
    ...
    for variant in VARIANTS[1:]:
        losses, full_rows, stats = train_variant(
            ...
            interface, embedding, args, device, target_names,
        )
    ...
    if len(set(lora_digests)) != 1:
        raise AssertionError(f"LoRA initialization differs across B/C/D: {lora_digests}")
    ...
    if any(order != expected_training_order for order in training_orders):
        raise AssertionError("Training case order differs across B/C/D")
    ...
    summary = {
        "stage": "V1 action ensemble hard pseudo-label upper bound",
        ...
        "formulas": {
            ...
            "C_wp_hard_fusion": "Y_C=1[(P_source+sum(P_WP_i))/7>0.5]",
            "D_real_action_hard_fusion": "Y_D=1[(P_source+sum(P_real_i))/7>0.5]",
        ...
        "training_protocol": {
            ...
            "lora_initialization_digests_BCD": lora_digests,
            "lora_initialization_consistent_BCD": len(set(lora_digests)) == 1,
            "training_order_consistent_BCD": all(
        ...
        "real_action_protocol": {
            "generation": "full raw case, one action at a time",
            "random_noise_bias": "V0 stable case/action/seed",
            "support_shape_topology_checks": True,
            "all_action_volumes_retained_simultaneously": False,
        ...
        "smoke_validation": {
            ...
            "C_pseudo_strict_binary_uint8": smoke_binary["C_wp_hard_fusion"],
            "D_pseudo_strict_binary_uint8": smoke_binary["D_real_action_hard_fusion"],
